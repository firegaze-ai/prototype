...
@torch.no_grad()
def load_weights(weights, imgsz, device, half=False):
    # Initialize
    set_logging()
    device = select_device(device)
    half &= device.type != 'cpu'  # half precision only supported on CUDA
...
def run_with_preloaded_weights(model,  # model.pt preloaded into memory
                               ...
                               names,  # class names
                               half,
                               classify=False,  # second-stage classifier flag
                               modelc=None,  # second-stage classifier model (if classify==True,
                               # modelc cannot be None)
                               imgsz=640,  # inference size (pixels)
                               conf_thres=0.25,  # confidence threshold
                               iou_thres=0.45,  # NMS IOU threshold
                               max_det=1000,  # maximum detections per image
